/*
  ESP32 Bluetooth Speaker — BT phase
  -----------------------------------
  - A2DP Bluetooth sink, audio out via internal DAC (GPIO25 = L, GPIO26 = R)
  - SSD1306 OLED (I2C on GPIO14/27) shows connection status, play/pause, track info
...
void updateDisplay() {
  ...
  // --- Track title, scrolling marquee if it doesn't fit ---
  ...
  if ((int)titleToShow.length() > VISIBLE_CHARS) {
    ...
    String padded = titleToShow + "   " + titleToShow; // wrap-around buffer
  ...
  display.print("Vol");
  int barX = 24, barY = 52, barW = 96, barH = 8;
  display.drawRect(barX, barY, barW, barH, SSD1306_WHITE);
  int fillW = map(volume, 0, 100, 0, barW - 2);
